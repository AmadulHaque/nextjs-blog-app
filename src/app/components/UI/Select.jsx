...
export default function Select({items, defaultValue = 0, onchange }) {
      ...
      <div className="relative ">
        <ListboxButton className="relative w-full cursor-default rounded-md bg-white py-1.5 pl-3 pr-10 text-left text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm sm:leading-6">
          <span className="flex items-center  rounded">
            {
                selected?.image ? <Image alt={selected?.title} src={selected?.image} className="h-5 w-5 flex-shrink-0 rounded-full"  width='5' height='5' /> : ""
            }        
        ...
        <ListboxOptions  transition  className="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none data-[closed]:data-[leave]:opacity-0 data-[leave]:transition data-[leave]:duration-100 data-[leave]:ease-in sm:text-sm">
          {items?.map((item,index) => (
            <ListboxOption  key={index}  value={item}  className="group relative cursor-default select-none py-2 pl-3 pr-9 text-gray-900 data-[focus]:bg-indigo-600 data-[focus]:text-white">
              <div className="flex items-center">
                {
                    item.image ?  <Image src={item.image} className="h-5 w-5 flex-shrink-0 rounded-full" width='5' height='5' alt={item.title} /> : ""
                }
